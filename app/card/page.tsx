...
export default async function CardPage() {
  ...
  if (error) return <Shell><p className="text-red-400">Failed to load card: {error.message}</p></Shell>;
  if (!data) {
    return (
      <Shell>
    ...
    <Shell>
      ...
      {card.picks.length > 0 ? (
        <Section title={`Picks (${card.picks.length})`}>
          <div className="grid gap-3">{card.picks.map((p, i) => <BetCard key={i} pick={p} />)}</div>
        </Section>
      ) : (
        <Section title="Picks">
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <main className="mx-auto max-w-2xl px-4 py-8 sm:py-12">
      <a href="/" className="text-xs text-zinc-400 hover:text-zinc-200">← Chat</a>
